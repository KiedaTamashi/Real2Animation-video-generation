...
class AnimeTrainDataset(Dataset):
    num_keypoints = 21
    ...
    def __init__(self, dataset_folder, stride, sigma, transform=None):
        ...
        self._labels = [line.rstrip('\n') for line in
                        open(os.path.join(self._dataset_folder, 'TrainVal_pose_annotations', 'anime_train_set.csv'), 'r')]
    ...
    def __getitem__(self, idx):
        ...
        # key points value should be modified when
        keypoints = np.ones(AnimeTrainDataset.num_keypoints*3, dtype=np.float32) * -1
        for id in range(keypoints.shape[0]//3):
            if tokens[1 + id*3] != 'nan':
                ...
                keypoints[id * 3 + 2] = 1                          # visible == 1, not visible == 0 TODO I am not sure waht set it all 1 affect
        ...
        sample['image'] = image.transpose((2, 0, 1)) #C,W,H
        return sample
    ...
    def _generate_keypoint_maps(self, sample):
        ...
        # heat maps. but resized
        keypoint_maps = np.zeros(shape=(AnimeTrainDataset.num_keypoints + 1,
        ...
        keypoint_maps[-1] = 1 - keypoint_maps.max(axis=0) #bg
...
class AnimeValDataset(Dataset):
    def __init__(self, dataset_folder, num_images=-1):
        super().__init__()
        self._dataset_folder = dataset_folder
        self.labels_file_path = os.path.join(self._dataset_folder, 'TrainVal_pose_annotations', 'anime_val_set.csv')
