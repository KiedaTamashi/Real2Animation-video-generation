...
class skeletonVAE(ptl.LightningModule):
    def __init__(self,in_channels,out_channels,latent_dim,hidden_dims,extract_layers,input_size,cond_size, kld_loss_weight,
                 cond_f_dims=128,block=BasicBlock):
        super(skeletonVAE, self).__init__()
        self.latent_dim = latent_dim
        self.input_size = input_size
        self.cond_f_dims = cond_f_dims
        self.loss_weight = kld_loss_weight # will multiply kld loss
        ...
        self.embed_condition = nn.Linear(self.cond_f_dims,input_size*input_size)
        ...
        self.fc_mu = nn.Linear(hidden_dims[-1] * 4, latent_dim) # TODO here is default 256. 256/64=4
        self.fc_var = nn.Linear(hidden_dims[-1] * 4, latent_dim)

        ######### decoder #########
        modules =[]
        self.decoder_input = nn.Linear(self.latent_dim + self.cond_f_dims, hidden_dims[-1] * 4)
        ...
        af_size = math.ceil(cond_size / 32 - 7 + 1)
        self.feature_extracter = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False),# size/2
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),#size/2
            self._make_layer(block, 64, extract_layers[0]),
            self._make_layer(block, 128, extract_layers[1], stride=2),# size/2
            self._make_layer(block, 256, extract_layers[2], stride=2),  # size/2
            self._make_layer(block, 512, extract_layers[2], stride=2),  # size/2
            nn.AvgPool2d(7, stride=1),
        )
        # transfer feature map b,512,h,w -> b,cond_f_dims
        self.feature_mapping = nn.Linear(512 * block.expansion * af_size * af_size, self.cond_f_dims)
    ...
    def forward(self, x, c):#input, condition
        ...
        embedded_condition = embedded_condition.view(-1,self.input_size,self.input_size).unsqueeze(1)
        embedded_input = self.embed_data(x)

        x_ = torch.cat([embedded_input, embedded_condition], dim = 1) # concat on channel dim
    ...
    def encode(self,x):
        ...
        result = torch.flatten(result,start_dim=1) #[N x C x H x W] -> [N,CHW]
    ...
    def _make_layer(self, block, channels, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.res_in_channels, channels, stride, downsample))
    ...
    def training_step(self, batch, batch_nb):
        ...
        return {'Train_loss': loss, 'Train_Reconstruction_Loss': recons_loss, 'Train_KLD': -kld_loss}
    ...
    def validation_step(self, batch, batch_nb):
        ...
        infer_out = [y_hat, mu, log_var]
        loss, recons_loss, kld_loss = self.my_loss(infer_out,y)
        return {'Val_loss': loss, 'Val_Reconstruction_Loss': recons_loss, 'Val_KLD': -kld_loss}

    def validation_end(self, outputs):
        avg_loss = torch.stack([x['Val_loss'] for x in outputs]).mean()
        avg_recon_loss = torch.stack([x['Val_Reconstruction_Loss'] for x in outputs]).mean()
        avg_kld_loss = torch.stack([x['Val_KLD'] for x in outputs]).mean()
        return {'avg_val_loss': avg_loss, 'avg_val_recon_Loss': avg_recon_loss, 'avg_val_KLD': -avg_kld_loss}
